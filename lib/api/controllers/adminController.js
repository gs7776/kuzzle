...
class AdminController {
  ...
  resetKuzzleData (request) {
    if (_locks.resetKuzzleData) {
      throw new BadRequestError('Kuzzle is already reseting his data.');
    }
    _locks.resetKuzzleData = true;
  ...
  resetSecurity (request) {
    if (_locks.resetSecurity) {
      throw new BadRequestError('Kuzzle is already reseting roles, profiles and users.');
    }
    _locks.resetSecurity = true;
  ...
  resetDatabase (request) {
    if (_locks.resetDatabase) {
      throw new BadRequestError('Kuzzle is already reseting all indexes.');
    }
    _locks.resetDatabase = true;

    const
      indexes = Object.keys(this.kuzzle.indexCache.indexes).filter(idx => idx[0] !== '%'),
      refresh = request.input.args.refresh;

    const promise = Bluebird.map(indexes, index => {
      const req = new Request({ index, refresh });
  ...
  /**
   * Generate a dump
   * Janitor will throw a BadRequestError if a dump is already running
   */
  dump (request) {
    const
      refresh = request.input.args.refresh,
      suffix = request.input.args.suffix ? request.input.args.suffix : '';

    const promise = this.kuzzle.janitor.dump(suffix);

    if (refresh === 'wait_for') {
      return promise.then(() => ({ acknowledge: true }));
    }
